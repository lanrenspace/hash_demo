 ...
 * @Description: 一致性ip_hash 哈希算法,含虚拟节点(解决数据倾斜问题)
 **/
public class ConsistentHashWithVirtual {
    ...
    public static void main(String[] args) {
        ...
        for (String tomcatServer : tomcatServers) {
            ...
            // 初始化虚拟节点
            for (int i = 0; i < virtualServerTotal; i++) {
                String virtualTomcatServer = tomcatServer + ".#" + i;
                int virtualServerHash = Math.abs(virtualTomcatServer.hashCode());
                hashServerMap.put(virtualServerHash, virtualTomcatServer);
            }
        }
        // 针对客户端ip算出hash值
